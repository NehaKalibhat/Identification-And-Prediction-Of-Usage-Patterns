...
if __name__ == '__main__':
	dataframe = pandas.read_csv("../Event Action Datasets From June 2016/MailRead_hours_March2017.csv", usecols=[5])
	rnn = KerasRNN(dataframe, 15)
	rnn.create_and_fit_LSTM(20)
	rnn.make_predictions()
	count_correct = 0
	for i in range(len(rnn.trainPredict)):
		if rnn.trainY[0][i]-rnn.trainPredict[i][0] < 5000:
			count_correct += 1
	for i in range(len(rnn.testPredict)):
		if rnn.testY[0][i]-rnn.testPredict[i][0] < 5000:
			count_correct += 1
