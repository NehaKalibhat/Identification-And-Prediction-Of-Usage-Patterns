...
class KerasRNN:
	def __init__(self, dataframe, look_back):
		...
		self.scaler_hour = MinMaxScaler(feature_range=(0, 1))
		self.scaler_weekday = MinMaxScaler(feature_range=(0, 1))
		self.scaler_monthday = MinMaxScaler(feature_range=(0, 1))

		self.dataset[:,3] = self.scaler_events.fit_transform(self.dataset[:,3])
		self.dataset[:,2] = self.scaler_hour.fit_transform(self.dataset[:,2])
		self.dataset[:,0] = self.scaler_weekday.fit_transform(self.dataset[:,0])
		self.dataset[:,1] = self.scaler_monthday.fit_transform(self.dataset[:,1])
	...
	def create_dataset(self, dataset):
		...
		for i in range(len(dataset)-self.look_back):
			...
			dataX.append(a)
			dataY.append(dataset[i + self.look_back, 3])
	...
	def plot_graph(self):
		...
		plt.plot(self.scaler_events.inverse_transform(self.dataset)[:,3], label="Actual Data")
		plt.plot(trainPredictPlot[:,3], label="Predictions on Training Dataset")
		plt.plot(testPredictPlot[:,3], label="Predictions on Test Dataset")
		plt.legend(loc='best')
		plt.show()
...
if __name__ == '__main__':
	dataframe = pandas.read_csv("../Event Action Datasets From June 2016/MailRead_hours_March2017.csv", usecols=[2,3,4,5], engine='python')
	rnn = KerasRNN(dataframe, 1)
	rnn.create_and_fit_LSTM(5)
	rnn.make_predictions()
	count_correct = 0

	for i in range(len(rnn.trainPredict)):
		if rnn.trainY[i]-rnn.trainPredict[i][0] < 5000:
			count_correct += 1
	for i in range(len(rnn.testPredict)):
		if rnn.testY[i]-rnn.testPredict[i][0] < 5000:
			count_correct += 1
